...
import 'config.dart';
/// A data model representing a single activity planned for a trip.
class Activity {
  String name;
  double cost;
...
/// A screen that allows users to create and manage a daily itinerary
/// for a selected date range, including adding activities and their costs.
...
class _ItineraryScreenState extends State<ItineraryScreen> {
  ...
  /// Initializes the itinerary map with empty lists for each day of the trip.
  ...
  /// Calculates the total cost of all activities planned for a specific day.
  ///
  /// Parameters:
  ///   dayNumber (int): The day of the trip to calculate the total for.
  double _calculateDayTotal(int dayNumber) {
    return _dayPlans[dayNumber]!.fold(0, (sum, item) => sum + item.cost);
  }

  /// Navigates to the search screen to find an activity. If a valid activity
  /// is selected, prompts the user to enter a cost for it.
  ...
  /// Displays a dialog for the user to input the cost of a discovered activity.
  ...
  /// Displays a dialog allowing the user to manually enter an activity's name and cost.
    ...
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        content: Column(
          ...
          children: [
            TextField(controller: nameController, decoration: const InputDecoration(hintText: "What's the plan?")),
            TextField(controller: costController, keyboardType: TextInputType.number, decoration: const InputDecoration(hintText: "Cost", prefixText: "BDT ")),
          ],
        ),
        actions: [
          ElevatedButton(
  ...
  /// Packages the structured itinerary data and sends a POST request
  /// to save it to the backend database.
  Future<void> _saveToDatabase() async {
    ...
    final url = Uri.parse("${AppConfig.baseUrl}/save_itinerary");
  ...
  /// Displays a success dialog upon successfully saving the itinerary to the database.
  ...
  /// Builds the top header row featuring a back button and title.
  Widget _buildHeader() {
    return Padding(
      padding: const EdgeInsets.all(20.0),
      child: Row(
  ...
  /// Builds a visual card for a specific day, listing activities, costs,
  /// and providing buttons to add new activities.
