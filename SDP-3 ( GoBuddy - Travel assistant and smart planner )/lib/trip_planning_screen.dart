import 'dart:async'; // Timer-er jonno proyojon
...
class _TripPlanningScreenState extends State<TripPlanningScreen> {
  ...
  List<Color> colorList = [
    const Color(0xFFE0F7FA), // Light Cyan (Thik kora hoyeche)
    const Color(0xFFF1F8E9), // Light Green (Thik kora hoyeche)
    const Color(0xFFFFF3E0), // Light Orange (Thik kora hoyeche)
    const Color(0xFFF3E5F5), // Light Purple (Thik kora hoyeche)
  ];
  int colorIndex = 0;
  Timer? timer;
  ...
  void initState() {
    super.initState();
    // Protikhon color change hobe smooth vabe
  ...
  @override
  void dispose() {
    timer?.cancel(); // Screen bondho hole timer stop hobe
    super.dispose();
  }

  // --- LOGIC: FETCH SAVED PLANS ---
  ...
  // --- HELPER: GET ITINERARIES (for FutureBuilder) ---
  ...
  // --- LOGIC: DELETE PLAN (Updated) ---
  Future<void> _deletePlanAndRefresh(int id, BuildContext modalContext) async {
    final url = Uri.parse("http://192.168.0.106:5000/delete_itinerary/$id");
    try {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      // Main background ekhon animated
      body: AnimatedContainer(
        duration: const Duration(seconds: 3), // Color transition speed
        curve: Curves.easeInOut,
        decoration: BoxDecoration(
          color: colorList[colorIndex], // Animated Color selection
        ),
        child: CustomScrollView(
          slivers: [
            SliverAppBar(
              ...
              pinned: true,
              backgroundColor: Colors.transparent, // AppBar transparent kora hoyeche
              elevation: 0,
              leading: IconButton(
